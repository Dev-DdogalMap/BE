...
public class ChatRoomsController {
    ...
    @Operation(
            summary = "그룹 채팅 메시지 조회V2(커서 기반)",
            description = "현재 로그인한 사용자가 참여 중인 그룹 채팅방의 최근 메시지를 조회합니다.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @GetMapping("/{roomId}/messages/v2")
    public ChatMessageCursorResponse getGroupChatMessagesV2(
            @AuthenticationPrincipal UserPrincipal principal,
            @PathVariable Long roomId,
            @RequestParam(required = false) Long cursorId,
            @RequestParam(required = false) Integer size
    ) {
        return chatRoomsService.getChatMessagesV2(principal.userId(), roomId, cursorId, size);
    }
